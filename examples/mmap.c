...
int main()
{
   ...
   uint32_t *FAT = (uint32_t *)((void *)fp + (MB->FatOffset * bytesPerSector)); // fat offset gives us a number in sectors, so multiply
   // by bytes per sector
   ...
   while (GDS[i].EntryType)
   {
      ...
      if (GDS[i].InUse && GDS[i].EntryType == 0x85)
      {
         ...
         struct FDE *fileDirectoryEntry = (struct FDE *)&GDS[i];
         if (fileDirectoryEntry->EntryType == 0x85)
         {
            printf("File Attributes: %x", fileDirectoryEntry->FileAttributes);
            printf("\n");
            // eat the next 2 directory entries
            i++; // eats the next file directory entry
            i++;
         }
      }
      i++;
   ...
   while (GDS[i].EntryType)
   {
      ...
      if (GDS[i].InUse && GDS[i].EntryType == 0xc0)
      {
         ...
         struct FSE *fileStreamExtension = (struct FSE *)&GDS[i];
         if (fileStreamExtension->EntryType == 0xc0)
         {
            printf("Cluster of Subdirectory: %x", fileStreamExtension->FirstCluster);
            printf("\n");
            // eat the next 2 directory entries
            i++; // eats the next file directory entry
            i++;
   ...
   while (GDS[i].EntryType)
   {
      ...
      if (GDS[i].InUse && GDS[i].EntryType == 0xc1)
      {
         ...
         if (X->EntryType == 0xc1)
         {
            printf("Size of struct: %ld", sizeof(X));
